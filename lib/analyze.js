...
Schema:
{
  "codeSummary":          string, required. 2-4 sentences in plain English describing what the submitted code appears to be. Include: the language/platform (e.g. "Arduino sketch", "Python script"), what the main entry point does at a high level, and what any imported libraries or helper files appear to do based on how they are called. Use hedging language ("appears to", "seems to") when you are not certain. Write for a non-technical reader — if you must use a technical term, follow it with a short plain-language gloss. DO NOT raise rule concerns in this field (those belong in the "questions" array). DO NOT use judgment words (no PASS, FAIL, COMPLIANT, VIOLATION, PROHIBITED, etc.). This field is required for every response, including when "questions" is empty.
...
If no patterns plausibly implicate the supplied RULES, return {"codeSummary": "<your summary>", "questions": []}. The codeSummary is ALWAYS required.
...
function normalizeResponse(parsed) {
  const codeSummary = parsed && typeof parsed.codeSummary === 'string'
    ? parsed.codeSummary.trim()
    : '';

  const questions = parsed && Array.isArray(parsed.questions)
    ? parsed.questions
...
async function analyze({ rulesText, codeBundle, codePaths, apiKey }) {
  ...
  const { codeSummary, questions } = normalizeResponse(parsed);

  return { codeSummary, questions, questionCount: questions.length };
}
